    ...
    public GUI(int columns, int rows) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                setSize(columns * width + 20, rows * height * 100 + 20);
                setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
                setLayout(new BoxLayout(f,BoxLayout.Y_AXIS));
                setBackground(Color.BLACK);
                progressBar.setStringPainted(true);
                list.setBounds(0,10,columns * width + 20, rows * height * 10);
                list.setLayout(new BoxLayout(list, BoxLayout.Y_AXIS));
                list.setBackground(Color.BLACK);
                scrollPane = new JScrollPane();
                scrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
                scrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
                scrollPane.setBounds(0, 0, columns * width + 20, rows * height * 10);
                scrollPane.setSize(columns * width + 20, rows * height * 10);
                scrollPane.getViewport().add(list);
                JPanel contentPane = new JPanel();
                contentPane.setLayout(new BoxLayout(contentPane,BoxLayout.Y_AXIS));
                contentPane.setPreferredSize(new Dimension(columns * width + 20, rows * height * 10));
